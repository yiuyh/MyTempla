...
using namespace std;
int a[20];
long long dp[20][20][4000];

long long dfs(int pos, int x, int sum, bool limit){
    if(pos < 0 || sum < 0){
        if(sum == 0) return 1;
        return 0;
    }
    if(!limit && dp[pos][x][sum] != -1) return dp[pos][x][sum];
    int up = limit? a[pos] : 9;
    long long res = 0;
    for(int i = 0; i <= up; i++){
        int ssum = sum+(pos-x)*i;
        res += dfs(pos-1, x, ssum, limit && i == a[pos]);
    }
    if(!limit)dp[pos][x][sum] = res;
    return res;
}
